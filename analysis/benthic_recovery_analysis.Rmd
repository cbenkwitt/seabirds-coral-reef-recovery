...
benthic_pred %>%
  ...
  geom_text(data = ann_text,label = ann_text$lab)+ #, hjust = 1
  theme_bw() + 
  theme(panel.grid.major = element_blank(), # remove gridlines
        panel.grid.minor = element_blank(), #remove gridlines
        strip.background = element_blank(), 
...
coral_recruit_cover_plot<-plot_grid(acrop_recruit_plot_mult_hdis, acrop_plot_mult_hdis,
                                    ...
                                    label_y = c(.99, .99, .99, .99), label_fontface = "plain")
...
benthic_distance_cover_plot<-plot_grid(
  ...
  nrow = 2,
  labels = "a.", rel_heights = c(.65, 1), label_x = .27, label_y =.99, label_fontface = "plain", label_size=12
)
...
nmds_plots<-plot_grid(nmds_coral_group_plot, nmds_benth_plot,
                                    ...
                                    labels = "auto", label_x = c(.12, .12), 
                                    label_y = c(.99, .99), label_fontface = "plain")
nmds_plots
