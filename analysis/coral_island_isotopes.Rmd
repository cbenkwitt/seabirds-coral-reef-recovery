...
rte_n15_mod %>% 
  ...
  ylab(expression(Symbiont~delta^15~N))+
  #guides(fill = guide_legend(override.aes = list(size = 0.5)), colour = 'none')+
  theme_bw() + 
  theme(panel.grid.major = element_blank(), # remove gridlines
        panel.grid.minor = element_blank(), #remove gridlines
        strip.background = element_blank(), 
...
island_n15_plots<-plot_grid(
  ...
  nrow = 2, labels = "AUTO",  rel_heights = c(.8, 1), 
  label_x = c(.37, .07), label_y = c(.96, .96), 
...
#sci advances:
ggsave(filename = "../output/figures/island_n15_plots.pdf", 
       plot = island_n15_plots,
       width = 7.25,
       height = 6,
       units = "in",
